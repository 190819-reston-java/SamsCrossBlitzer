...
"XLNX","XYL","YUM","ZBH","ZION","ZTS"
  ...
  ngOnInit() {  
    this.pricingData.getPrices("P").subscribe(post => {
      this.post  = post;
    })
      
      this.newsData.getNews("P").subscribe(postNews => {
    ...
    private updateChart(option){
      this.pricingData.getPrices(option).subscribe(post => {
        this.post  = post;
      })        
